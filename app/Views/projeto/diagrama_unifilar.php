...
<body>
    ...
    // A. Formatação da String de Fases da Entrada (Para gerar I, II ou III) qtd_fase
    $strFasesEntrada = 'A'; // Padrão
    if (isset($tipo_geral)) {
        if ($tipo_geral == 2) $strFasesEntrada = 'AB';
        if ($tipo_geral == 3) $strFasesEntrada = 'ABC';
    }

    // B. Formatação do Cabo de Entrada (Ex: 3#25(25)mm²)
    // $fase_mm = $entrada_fase ?? '0';
    // $neutro_mm = $entrada_neutro ?? '0';
    $txtCaboEntrada = ($qtd_fase == 1)
        ? "{$tipo_geral}#{$entrada_fase}({$entrada_neutro})mm²"
        : "{$qtd_fase}x{{$tipo_geral}#{$entrada_fase}({$entrada_neutro})}mm²";
    //"{$tipo_geral}#{$fase_mm}({$neutro_mm})mm²";
    ...
    $dadosEntradaJS = [
        ...
        'tuboEntrada'     => 'Ø' . $entrada_eletroduto . '"' ?? '',
    ...
    if (!empty($unidades) && is_array($unidades)) {
        foreach ($unidades as $index => $u) {
            ...
            // Pega os novos dados separados
            $qtdPorFase = $u['qtd_fase'] ?? '1';
            $qtdFases = mb_strlen($u['fases_especificas'], 'UTF-8');
            $fase    = $u['fase'] ?? '';
            $neutro  = $u['neutro'] ?? '';

            $strCabo = '';
            if ($fase !== '') {
                $strCabo = ($qtdPorFase == 1)
                    ? "{$qtdFases}#{$fase}({$neutro})mm²"
                    : "{$qtdPorFase}x{{$qtdFases}#{$fase}({$neutro})}mm²";
            }
            $unidadesJS[] = [
    ...
    <div style="text-align: center; max-width: 800px; margin: 0 auto;" class="no-print">
